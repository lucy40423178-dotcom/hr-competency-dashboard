...
직무 선택 → 요약 지표 카드 + 요구/보유 비교 + 구성원 갭 히트맵 + 교육 추천
"""
import numpy as np
import pandas as pd
...
h1, h2, h3 { color:#FAFAFA !important; letter-spacing:-0.5px; }
.metric-card {
  background:linear-gradient(135deg,#1A1D26 0%,#22262F 100%);
  border:1px solid #2A2E38; border-radius:16px; padding:18px 20px;
  box-shadow:0 4px 16px rgba(0,0,0,0.35);
}
.metric-card .label { color:#8A8F9C; font-size:13px; margin-bottom:6px; }
.metric-card .value { color:#1D9E75; font-size:30px; font-weight:700; line-height:1.1; }
...
cmp_df = pd.DataFrame({"요구역량": req_vec, "보유역량(평균)": 보유_all.loc[job]})
st.bar_chart(cmp_df, height=300, color=["#1D9E75", "#5B8DEF"])

st.markdown('<span class="section-tag">STEP 2</span>', unsafe_allow_html=True)
st.markdown("#### 구성원별 역량 갭 (요구 − 보유) · 양수=부족")
st.dataframe(
    gap_df.style.background_gradient(cmap="RdBu_r", vmin=-1.5, vmax=1.5, axis=None).format("{:.1f}"),
